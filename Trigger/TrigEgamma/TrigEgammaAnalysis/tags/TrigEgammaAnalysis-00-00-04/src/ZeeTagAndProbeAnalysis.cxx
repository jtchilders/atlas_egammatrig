...
StatusCode TrigEgammaZeeTagAndProbeAnalysis::BookHistos(void){
   ...
   sc = m_histoSvc->regHist(str_L2vsOffDir,m_h2f_probe_noL2e_etaPhi);
   if(sc.isFailure()){
   ...
   std::string regName = outputBaseDir + m_h1f_inv_mass_ee->GetName();
      ...
      for(unsigned int j=0;j<m_probeList->size();++j){
         ...
         *m_log << MSG::INFO << "Number of Probe Electrons: " << m_probeList->size() << endreq;
         ...
         if(m_trigMatch->chainPassedByObject<TrigMatch::TrigElectronL2>(probe,"L2_e20_loose",0.15)){
            ...
            // fill L2 plots for L2_e20_loose
            std::vector<const TrigMatch::TrigElectronL2*> trigElectrons = m_trigMatch->matchToTriggerObjects<TrigMatch::TrigElectronL2>(probe,"L2_e20_loose",0.15);
            
            const TrigMatch::TrigElectronL2 *electronMatch = GetMatchedL2Electron(probe,trigElectrons);
            // if matched electron was found
            if(electronMatch != 0){
               if(80 <= invmass && invmass <= 95){
                  m_trigE_probe_L2_e20_loose_Zee->Fill(electronMatch);
                  m_l2vsOff_probe_L2_e20_loose_Zee->Fill(electronMatch,probe);
               }
               else{
                  m_trigE_probe_L2_e20_loose->Fill(electronMatch);
                  m_l2vsOff_probe_L2_e20_loose->Fill(electronMatch,probe);
               }
            }
...
const TrigMatch::TrigElectronL2* TrigEgammaZeeTagAndProbeAnalysis::GetMatchedL2Electron(Analysis::Electron *electron,std::vector<const TrigMatch::TrigElectronL2*>& trigElectrons)
{
   // setup msg stream for output
   m_log = new MsgStream(msgSvc(), name());
